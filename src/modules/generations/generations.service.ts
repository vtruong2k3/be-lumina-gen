...
export class GenerationsService {
  ...
  async createGeneration(
    ...
    // Auto-trim: keep only MAX_GENERATIONS most recent
    const count = await this.prisma.generation.count({ where: { userId } });
    if (count > MAX_GENERATIONS) {
      const oldest = await this.prisma.generation.findMany({
        where: { userId },
